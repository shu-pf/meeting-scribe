...
- 録画音声を変更したときは、実録画ファイルにシステム音声とマイク音声の2トラックがあり、両方に実データが入ること、および文字起こし用WAVへ正常に合成できることを確認する
...
- 録画にはシステム音声とマイク音声（自分の声）を含める。録画ファイルでは別トラックとして保持し、文字起こし時に全音声トラックをモノラルへ合成する
- 録画開始には、選択済みWhisperモデルの実ファイルが必要。未導入時は録画を無効化してダウンロードを案内し、存在しない `default` モデルへフォールバックしない
